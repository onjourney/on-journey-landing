...
<nav :class="[isActiveNav ? 'bg-white shadow-cs-50' : 'bg-cs-dark-blue']" class="sticky top-0 z-[100] transition-colors duration-300 min-h-[78px]">
    <div class="lg:max-w-[1320px] xl:max-w-[1536px] mx-auto flex justify-between py-4 px-5 sm:px-15 lg:px-32">
        <div class="inline-block relative transition-all duration-300 min-h-[22.5px]" :class="[isActiveNav ? 'bottom-0' : 'bottom-[-6px] md:bottom-[-16px]']">
        ...
        <div class="md:hidden flex gap-3 items-center">
            ...
            <!-- burger -->
            <button @click="$parent.isNavMobileActive = true" class="inline-block px-2 py-1.5 rounded-md transition-colors duration-300" :class="[isActiveNav ? 'text-cs-text-primary bg-black bg-opacity-[0.05]' : 'text-white bg-white bg-opacity-20']">
